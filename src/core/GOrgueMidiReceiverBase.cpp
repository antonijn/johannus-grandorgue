...
void GOrgueMidiReceiverBase::SetElementID(int id)
{
	m_ElementID = id;
}

const struct IniFileEnumEntry GOrgueMidiReceiverBase::m_MidiTypes[] = {
...
void GOrgueMidiReceiverBase::Preconfigure(GOrgueConfigReader& cfg, wxString group)
{
}
...
bool GOrgueMidiReceiverBase::HasChannel(midi_match_message_type type)
{
	if(type == MIDI_M_NOTE ||
	   ...
	   type == MIDI_M_CTRL_CHANGE_FIXED_ON_OFF ||
	   type == MIDI_M_RPN_ON ||
	   type == MIDI_M_RPN_OFF ||
	   type == MIDI_M_RPN_ON_OFF ||
	   type == MIDI_M_NRPN_ON ||
	   type == MIDI_M_NRPN_OFF ||
	   type == MIDI_M_NRPN_ON_OFF ||
...
bool GOrgueMidiReceiverBase::HasKey(midi_match_message_type type)
{
	if(type == MIDI_M_NOTE ||
	   type == MIDI_M_CTRL_CHANGE ||
	   type == MIDI_M_PGM_CHANGE ||
...
bool GOrgueMidiReceiverBase::HasLowKey(midi_match_message_type type)
{
	if (m_type != MIDI_RECV_MANUAL)
		return false;
	if(type == MIDI_M_NOTE ||
...
bool GOrgueMidiReceiverBase::HasHighKey(midi_match_message_type type)
{
	if (m_type != MIDI_RECV_MANUAL)
		return false;
	if(type == MIDI_M_NOTE ||
...
bool GOrgueMidiReceiverBase::HasDebounce(midi_match_message_type type)
{
	if (m_type == MIDI_RECV_MANUAL)
		return false;
	if (m_type == MIDI_RECV_ENCLOSURE)
...
bool GOrgueMidiReceiverBase::HasLowerLimit(midi_match_message_type type)
{
	if (type == MIDI_M_NOTE ||
	    type == MIDI_M_PGM_RANGE ||
	    type == MIDI_M_RPN_RANGE ||
...
bool GOrgueMidiReceiverBase::HasUpperLimit(midi_match_message_type type)
{
	if (type == MIDI_M_NOTE ||
	    type == MIDI_M_PGM_RANGE ||
	    type == MIDI_M_RPN_RANGE ||
...
unsigned GOrgueMidiReceiverBase::KeyLimit(midi_match_message_type type)
{
	if (type == MIDI_M_PGM_CHANGE)
		return 0x200000;
...
MIDI_MATCH_TYPE GOrgueMidiReceiverBase::debounce(const GOrgueMidiEvent& e, MIDI_MATCH_TYPE event, unsigned index)
{
...
void GOrgueMidiReceiverBase::deleteInternal(unsigned device)
{
...
unsigned GOrgueMidiReceiverBase::createInternal(unsigned device)
{
	unsigned pos = 0;
...
MIDI_MATCH_TYPE GOrgueMidiReceiverBase::Match(const GOrgueMidiEvent& e)
{
	int tmp;
	return Match(e, tmp);
}

MIDI_MATCH_TYPE GOrgueMidiReceiverBase::Match(const GOrgueMidiEvent& e, int& value)
{
	int tmp;
	return Match(e, NULL, tmp, value);
}

MIDI_MATCH_TYPE GOrgueMidiReceiverBase::Match(const GOrgueMidiEvent& e, const unsigned midi_map[128], int& key, int& value)
{
	value = 0;
	...
	for(unsigned i = 0; i < m_events.size();i++)
	{
		...
		if (e.GetMidiType() == MIDI_NOTE && m_events[i].type == MIDI_M_NOTE_OFF && m_events[i].key == e.GetKey() && e.GetValue() <= m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_NOTE && m_events[i].type == MIDI_M_NOTE_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() >= m_events[i].high_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_NOTE && m_events[i].type == MIDI_M_NOTE_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() <= m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		...
		if (e.GetMidiType() == MIDI_CTRL_CHANGE && m_events[i].type == MIDI_M_CTRL_CHANGE_OFF && m_events[i].key == e.GetKey() && e.GetValue() <= m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_CTRL_CHANGE && m_events[i].type == MIDI_M_CTRL_CHANGE_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() >= m_events[i].high_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_CTRL_CHANGE && m_events[i].type == MIDI_M_CTRL_CHANGE_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() <= m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		...
		if (e.GetMidiType() == MIDI_CTRL_CHANGE && m_events[i].type == MIDI_M_CTRL_CHANGE_FIXED_OFF && m_events[i].key == e.GetKey() && e.GetValue() == m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_CTRL_CHANGE && m_events[i].type == MIDI_M_CTRL_CHANGE_FIXED_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() == m_events[i].high_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_CTRL_CHANGE && m_events[i].type == MIDI_M_CTRL_CHANGE_FIXED_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() == m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		...
		if (e.GetMidiType() == MIDI_RPN && m_events[i].type == MIDI_M_RPN_OFF && m_events[i].key == e.GetKey() && e.GetValue() <= m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_RPN && m_events[i].type == MIDI_M_RPN_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() >= m_events[i].high_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_RPN && m_events[i].type == MIDI_M_RPN_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() <= m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		...
		if (e.GetMidiType() == MIDI_NRPN && m_events[i].type == MIDI_M_NRPN_OFF && m_events[i].key == e.GetKey() && e.GetValue() <= m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_NRPN && m_events[i].type == MIDI_M_NRPN_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() >= m_events[i].high_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		if (e.GetMidiType() == MIDI_NRPN && m_events[i].type == MIDI_M_NRPN_ON_OFF && m_events[i].key == e.GetKey() && e.GetValue() <= m_events[i].low_value)
			return debounce(e, MIDI_MATCH_CHANGE, i);
		...
		if (e.GetMidiType() == MIDI_PGM_CHANGE && m_events[i].type == MIDI_M_PGM_RANGE && m_events[i].high_value == e.GetKey())
				return MIDI_MATCH_ON;
		if (e.GetMidiType() == MIDI_SYSEX_JOHANNUS_9 && m_events[i].type == MIDI_M_SYSEX_JOHANNUS_9 && m_events[i].key == e.GetKey())
		{
			return debounce(e, MIDI_MATCH_CHANGE, i);
		}
		if (e.GetMidiType() == MIDI_SYSEX_JOHANNUS_11 && m_events[i].type == MIDI_M_SYSEX_JOHANNUS_11 && m_events[i].key == e.GetKey() && m_events[i].low_value <= e.GetValue() && m_events[i].high_value >= e.GetValue())
		{
			return debounce(e, MIDI_MATCH_CHANGE, i);
		}
...
void GOrgueMidiReceiverBase::Assign(const GOrgueMidiReceiverData& data)
{
	*(GOrgueMidiReceiverData*)this = data;
}

void GOrgueMidiReceiverBase::PreparePlayback()
{
	m_Internal.resize(0);
}
